...
const TerminalFrame = forwardRef<HTMLDivElement, Props>(
  ({ title, children, className = "" }, ref) => {
    ...
    return (
      <div
        ref={(node) => { frameRef.current = node; scrollRef.current = node; }}
      ...
      >
        ...
        <div className="relative flex items-center gap-2 px-4 py-2 border-b border-[#2a2a2a] bg-[#161616] overflow-hidden">
          ...
          <span className="relative z-10 w-3 h-3 rounded-full bg-[#FF6600] opacity-75 shrink-0" />
          <span className="relative z-10 w-3 h-3 rounded-full bg-[#222] shrink-0" />
          <span className="relative z-10 w-3 h-3 rounded-full bg-[#222] shrink-0" />
          <span className="relative z-10 ml-3 text-[10px] uppercase tracking-[0.25em] text-[#666] font-[family-name:var(--font-space-mono)]">
            {title}
          </span>
        </div>
